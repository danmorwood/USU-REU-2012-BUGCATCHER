...
/////constants//////
define(ROOT_DIRECTORY, 'C:\Dropbox\htdocs\NewDesign\OldCode'); //the root directory of the website
///////////////////
...
$contentFileName = "${compID}${teamName}Content.txt";//name of the content file
...
$bugFileLoc = "Competitions/${compID}${teamName}${problemName}Bugs.txt";
...
foreach($buggyOutputs as $index => $bO)
{
	...
	//bug found!
	if($bO != $oracle && !isLineInFile($bugName, $bugFileLoc))
	{
		/*
		...
		*/
		...
		mySQLUpdate("UPDATE ${compID}teams SET bugsFound=bugsFound+1 WHERE teamname='${teamName}'");
		...
		mySQLUpdate("UPDATE ${compID}teams SET totaltimefound=totaltimefound+${currentTime} WHERE teamname = '${teamName}'");
